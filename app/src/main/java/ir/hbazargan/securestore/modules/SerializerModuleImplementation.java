...
public final class SerializerModuleImplementation implements SerializerModuleContract {
  ...
  public SerializerModuleImplementation(LogInterceptorModuleContract logInterceptorModuleContract) {
  ...
  public <T> String serialize(String cipherText, T originalGivenValue) {
    Utils.checkNullOrEmpty("Cipher text", cipherText);
    Utils.checkNull("Value", originalGivenValue);
  ...
  public DataInfo deserialize(String serializedText) {
    ...
    if (firstElement != null && firstElement.length() != 0) {
      try {
        ...
        logInterceptorModuleContract.onLog("SerializerModuleImplementation -> " + e.getMessage());
      }
    }
